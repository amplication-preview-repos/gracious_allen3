...
export class ItemResolverBase {
  ...
  async createItem(@graphql.Args() args: CreateItemArgs): Promise<Item> {
    return await this.service.createItem({
      ...
      data: {
        ...
        receipt: args.data.receipt
          ? {
              connect: args.data.receipt,
            }
          : undefined,
      },
    });
  ...
  @graphql.ResolveField(() => Receipt, {
    nullable: true,
    name: "receipt",
  })
  async getReceipt(@graphql.Parent() parent: Item): Promise<Receipt | null> {
    const result = await this.service.getReceipt(parent.id);
